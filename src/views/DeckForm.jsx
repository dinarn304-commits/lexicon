...
export default function DeckForm({ initialDeck, onSave, onCancel }) {
  const editing = !!initialDeck;
  const [name, setName] = useState(initialDeck?.name ?? '');
  const [description, setDescription] = useState(initialDeck?.description ?? '');
  const [language, setLanguage] = useState(initialDeck?.language ?? '');

  function submit() {
    if (!name.trim()) return;
    if (editing) {
      onSave({ ...initialDeck, name: name.trim(), description: description.trim(), language: language.trim() });
    } else {
      onSave({
        id: makeId(),
        name: name.trim(),
  ...
  return (
    <div className="max-w-xl mx-auto px-6 py-10 fade-up">
      ...
      <div className="ornament text-xs mb-3">{editing ? '· EDIT DECK ·' : '· NEW DECK ·'}</div>
      <h1 className="display text-3xl mb-6">{editing ? 'Revise this deck' : 'A fresh page'}</h1>
